...
- 词条增删改查（Alconna 命令）
- 消息模板匹配与变量渲染
- 逻辑控制符（`or/and/not/xor/in/not in/==/!=/<=/>=/if/else`）
- 随机模板（随机数、随机选择）
...
`question` 侧赋值（表达式支持随机数、时间模板、消息段字段和字面量）：
...
- `[or] [and] [not] [xor] [in] [not in] [==] [!=] [<=] [>=] [if] [else]`
...
- `question=[if][text.text][==]你好[a=命中][else][a=未命中]`
- `answer=[a]`
- `question=[if][text.text][==]你好`（仅条件判断，命中才触发）

### 7. 等待输入模板

- 语法：`[await.<秒数>.<变量名>]`
- 可用于 `question` 和 `answer` 的任意位置
- 默认提示词：`请输入内容`
- 示例：`answer=文本1[await.10.text][text]`
